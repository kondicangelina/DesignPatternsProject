...
public class DialogUpdateSquare extends JDialog{
...
public DialogUpdateSquare() {
		...
		if (txtXCoord.getText().isEmpty()) {
			...
			try {
				...
				if (xCoordinate <= 0) {
					...
					square = new Square(new Point(xCoordinate, yCoordinate), width, btnColor.getBackground(), btnInnerColor.getBackground());
					dispose();
				}
			} catch (Exception e) {
	...
	public void fillFields(Square square) {
		txtXCoord.setText(String.valueOf(square.getUpperLeftPoint().getX()));
		txtYCoord.setText(String.valueOf(square.getUpperLeftPoint().getY()));
		txtWidth.setText(String.valueOf(square.getWidth()));
