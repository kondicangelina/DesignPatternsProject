...
public class DialogUpdateCircle extends JDialog{
	...
	public void fillFields(Circle circle) {
		txtXCoord.setText(String.valueOf(circle.getCenterPoint().getX()));
		txtYCoord.setText(String.valueOf(circle.getCenterPoint().getY()));
		...
		if (txtXCoord.getText().isEmpty()) {
			...
			try {
				...
				if (xCoordinate <= 0) {
					...
					circle = new Circle(new Point(xCoordinate, yCoordinate), radius, btnColor.getBackground(), btnInnerColor.getBackground());
					dispose();
				}
			} catch (Exception e) {
	...
	public void generateUserInterface() {
		...
		gbl_mainPanel.rowHeights = new int[]{30, 16, 23, 0, 22, 29, 0, 0, 0, 0, 0, 0, 0};
		...
		lblCenterPoint = new JLabel("Center point");
		...
		mainPanel.add(lblCenterPoint, gbc_lblUpLeftPoint);
		...
		mainPanel.add(lblRadius, gbc_lblWidth);
		...
		mainPanel.add(txtRadius, gbc_txtWidth);
